...
function startPreciseTimer(duration) {
  if (timerRequestId) {
...
(function() {
  window.addEventListener("message", function(event) {
    // Verify the event origin to ensure security
    ...
    if (event.data && event.data.type === "SYNC_CODE") {
      // Relay the sync code to the background script
      browser.runtime.sendMessage({ action: "syncCodeGenerated", syncCode: event.data.syncCode })
        .then(response => {
